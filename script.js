  ...
  async function syncFromCloud() {
    ...
    async function saveProduct() {
      ...
      try {
        ...
        if (editingProductId && editingProductId !== id) {
          showStatus(`Removing old record: ${editingProductId}...`);
          
          // Use the correct variable name here: deleteResponse
          const deleteResponse = await InventoryDB.deleteProduct(editingProductId);
          
          if (!deleteResponse.ok) {
            console.error("The old row was NOT deleted. Check RLS policies.");
          } else {
            console.log("Old row successfully purged.");
        ...
        // 2. Extra Safety: Delete any existing row that has this SAME Model Number 
        // but perhaps a different barcode (this prevents the "Still There" issue)
        await InventoryDB.deleteProductByModel(p.modelNumber);
      ...
      try {
        ...
        await Promise.all([
          ...
          InventoryDB.addTransaction({
            ...
            poNumber: po || '', 
            project: proj,
            projectAction: action === 'add' ? 'RESERVE' : 'RETURN',
            when: customDate ? new Date(customDate).toISOString() : new Date().toISOString()
          })
        ]);
        const freshTransactions = await InventoryDB.getRecentTransactions();
        if (Array.isArray(freshTransactions)) {
            transactions = freshTransactions; // Update your global variable
        }
    ...
    async function saveBulkProducts(type) {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (!model || qty <= 0) {
          ...
          if (qty <= 0) {
            qtyInput.style.border = '2px solid var(--danger)'; // Now qtyInput is definitely defined
      ...
      for (const r of rows) {
        ...
        const tx = {
          ...
          poNumber: r.querySelector('.bulk-po').value.trim() || '',
          project: r.querySelector('.bulk-project')?.value.trim() || '', 
      ...
      updateAllProductsView();
      renderProjectsTab();
      if (typeof updateTransactionHistory === 'function') transactions = await InventoryDB.getRecentTransactions(); updateTransactionHistory();

      type === 'in' ? closeBulkInModal() : closeBulkOutModal();
    ...
    function generatePOOverview() {
      ...
      // Filter logic: Check poNumber (new) and po_number/ref (old/fallback)
      const list = transactions.filter(tx => {
        const poValue = tx.poNumber || tx.po_number || "";
        // Only include if poValue is not empty
        if (!poValue) return false;
        // If there is a search term, check if it matches
        return poValue.toLowerCase().includes(search);
      });

      if (list.length === 0) {
        content.innerHTML = '<div class="small">No matching PO transactions.</div>';
        return;
      }
      ...
      list.forEach(tx => {
        // 1. Fix Invalid Date: Use created_at (Supabase default)
        const dt = new Date(tx.created_at || tx.when);
        
        // 2. Fix Product Name: Use displayName from your new schema
        const product = tx.displayName || tx.productId;
        
        // 3. Fix Undefined Qty: Use stock_change (underscore) as seen in your DB
        const qty = (tx.stock_change !== undefined) ? tx.stock_change : (tx.stockChange || 0);
        
        // 4. Fix After Stock: Use stockAfter from your new schema
        const after = tx.stockAfter;
        
        // 5. Fix PO Display
        const poDisplay = tx.poNumber || tx.po_number || tx.ref || '';

        html += `<tr>
              <td>${escapeHtml(poDisplay)}</td>
    ...
    function updateTransactionHistory() {
      ...
      const poFilter = (document.getElementById('tx-filter-po').value || '').toLowerCase();
      const modelFilter = (document.getElementById('tx-filter-model').value || '').toLowerCase();

      const list = transactions.filter(tx => {
        // Check both potential field names for PO Number and Product Name
        const txPo = (tx.poNumber || tx.po_number || tx.ref || '').toLowerCase();
        const txName = (tx.displayName || tx.display_name || tx.productId || tx.barcode || '').toLowerCase();

        if (poFilter && !txPo.includes(poFilter)) return false;
        if (modelFilter && !txName.includes(modelFilter)) return false;
        return true;
      });

      if (list.length === 0) {
        container.innerHTML = '<div class="small">No transactions found.</div>';
        return;
      }
      ...
      list.forEach(tx => {
        // 1. Map Date (Supabase returns created_at)
        const dt = new Date(tx.created_at || tx.when);
        
        // 2. Map Product Name
        const product = tx.displayName || tx.display_name || tx.barcode || 'Unknown';
        
        // 3. Map Numbers (Handles snake_case from DB and camelCase from JS)
        const change = (tx.stock_change !== undefined) ? tx.stock_change : (tx.stockChange || 0);
        const after = (tx.stockAfter !== undefined) ? tx.stockAfter : (tx.stock_after !== undefined ? tx.stock_after : '-');
        
        // 4. Map Project Details
        const project = tx.project || tx.project_name || '';
        const action = tx.projectAction || tx.project_action || '';
        const po = tx.poNumber || tx.po_number || tx.ref || '';
    ...
    async function restoreAll() {
      ...
      try {
        ...
        // --- NEW THING: ID NORMALIZATION ---
        // This ensures the restored data follows the Model-Suffix rule
        const cleanedProducts = {};
        const rawProducts = data.products || {};
        
        for (const oldKey in rawProducts) {
          const p = rawProducts[oldKey];
          // Use our helper to get the correct ID based on the actual model/suffix fields
          const newId = getProductId(p); 
          cleanedProducts[newId] = p;
        }

        // 1. Sync Primary Data Sets (Using cleanedProducts)
        await Promise.all([
          InventoryDB.bulkPutProducts(cleanedProducts), // Use cleaned version here
          InventoryDB.saveAllProjects(data.projectAllocations || {}),
          InventoryDB.saveCategories(data.categories || ['Default'])
        ]);
        
        // 2. Transaction Log (Keep as is)
        const txToRestore = (data.transactions || []).slice(0, 50);
        for (const tx of txToRestore) {
          await InventoryDB.addTransaction(tx);
        }

        // 3. Update local state (Using cleaned version)
        productDatabase = cleanedProducts; 
    ...
    function searchByRemark() {
      ...
      filtered.forEach(tx => {
        // Look up by the ID stored in the transaction
        const p = productDatabase[tx.productId] || {};
        const row = document.createElement("tr");
        row.innerHTML = `
          <td>${tx.when ? new Date(tx.when).toLocaleString() : ""}</td>
          // Show the Display Name (Model-Suffix) instead of just the ID
          <td>${escapeHtml(getProductDisplayName(p) || tx.productId)}</td>
          <td>${escapeHtml(p.category || "")}</td>
          <td>${tx.stockChange > 0 ? '+' + tx.stockChange : tx.stockChange}</td>
          <td>${escapeHtml(tx.reason || "")}</td>
          <td>${escapeHtml(tx.ref || tx.poNumber || "")}</td>
        `;
        tbody.appendChild(row);
      });
    }
      function handleResetConnection() {
        const confirmed = confirm("Are you sure? This will remove your Supabase credentials and you will need to enter them again to access the data.");
        
        if (confirmed) {
          // This calls the function already defined in your db.js
          InventoryDB.resetKeys();
        }
      }
    // ============ Init ============
    async function init(){
      ...
      updateAllProductsView();
      renderProjectsTab();
      transactions = await InventoryDB.getRecentTransactions();
      updateTransactionHistory();
    }
